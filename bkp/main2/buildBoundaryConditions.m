...
for idx = 1:length(BC)
	if strcmp(BC(idx).type, 'TEMPERATURE')
		fdisp(program.logger, ['Applying temperature boundary condition, ', num2str(BC(idx).value(1)), ' ºC']);
	...
	elseif strcmp(BC(idx).type, 'HEATFLOW')
		fdisp(program.logger, ['Applying heat flow boundary condition, ', num2str(BC(idx).value), ' W']);
	...
	elseif strcmp(BC(idx).type, 'HEATFLUX')
		fdisp(program.logger, ['Applying heat flux boundary condition, ', num2str(BC(idx).value), ' W/mm²']);
	...
	elseif strcmp(BC(idx).type, 'CONVECTION')
		fdisp(program.logger, ['Applying convection boundary condition, ', num2str(BC(idx).value(1)), ' W/mm²ºC, ', num2str(BC(idx).value(2)), ' ºC']);
    ...
    if strcmp(BC(idx).elementType, 'ELEM')
      fdisp(program.logger, ['Applying internal heat generation condition, ', num2str(BC(idx).value), ' W/mm³']);
      for i = 1:4
    ...
    elseif strcmp(BC(idx).elementType, 'NODE')
      ...
      fdisp(program.logger, ['Failed to apply boundary condition: Unknown element type "', BC(idx.elementType), '"']);
    end
